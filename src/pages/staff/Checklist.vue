...
const tabs = computed(() => [
  { key: 'real_stuff', label: t('categories.inventory.real_stuff'), count: inventoryStore.realStuff.length },
  { key: 'peripherals', label: t('categories.inventory.peripherals'), count: inventoryStore.peripherals.length },
])
...
<template>
  <div>
    ...
    <div v-else-if="sortedList.length === 0">
      <AppEmptyState :title="t('checklist.noItems')">
